...
import { ctx, VIEW_WIDTH } from "./config";
import { drawPanel, drawText, Settings } from "./theme";
import { tweenProgress, easeOutBack } from "@dark-fantasy/sim";
...
export function showBanner(text: string, sub?: string, duration?: number) {
    banners.push({
        text: text,
        sub: sub || "",
        timer: duration || 2.5,
...
export function updateBanners(dt: number) {
