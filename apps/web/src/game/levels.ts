...
import { TILE, ctx, VIEW_WIDTH, VIEW_HEIGHT } from "./config";
import { isColliding as rectCollides, LEVELS, isInView, getAnimationFrame, type GridMap, type LevelDef, type Rect, type Vec2 } from "@dark-fantasy/sim";
import { camera } from "./state";
import { Assets } from "./assets";

export { aabb } from "@dark-fantasy/sim";
export { LEVELS };

export let levelIndex = 0;
export let currentLevel: LevelDef | null = null;
export let currentMap: GridMap = [];
export let solidObjects: Rect[] = [];

let currentTorches: Vec2[] = [];
let mapCanvas: HTMLCanvasElement | null = null;
let mapCtx: CanvasRenderingContext2D | null = null;

export function buildLevel(index: number) {
    levelIndex = index;
...
export function isColliding(x: number, y: number, width: number, height: number) {
...
function drawGrassTile(gctx: CanvasRenderingContext2D, x: number, y: number, p: LevelDef["palette"]) {
...
function drawStoneTile(gctx: CanvasRenderingContext2D, x: number, y: number, p: LevelDef["palette"]) {
...
function drawTree(gctx: CanvasRenderingContext2D, x: number, y: number) {
...
function drawRock(gctx: CanvasRenderingContext2D, x: number, y: number) {
    gctx.fillStyle = "rgba(0, 0, 0, 0.35)";
...
function drawGrass(gctx: CanvasRenderingContext2D, x: number, y: number) {
...
function drawTorch(gctx: CanvasRenderingContext2D, x: number, y: number, time: number) {
...
function drawTorchSprite(gctx: CanvasRenderingContext2D, x: number, y: number, time: number) {
...
export function drawMap(time: number) {
    if (!mapCanvas) {
        return;
    }
...
export function drawTorchLights(time: number) {
