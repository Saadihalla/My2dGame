...
import { canvas, ctx, VIEW_WIDTH, VIEW_HEIGHT, DASH_COOLDOWN } from "./config";
import { gameState, gameTime, stats, wave, waveState, waveTimer, highScore, newHighScore } from "./state";
import { currentPlayer } from "./auth";
import { player, enemies, ENEMY_TYPES } from "./entities";
import {
    COLORS,
    drawPanel,
    drawGradientBar,
...
export function setButtons(list: UIButton[]) {
    uiButtons = list;
}
...
export function makeCard(label: string, desc: string, key: number | string, x: number, y: number, w: number, h: number, action: () => void, id?: string): UIButton {
    return {
        x: x,
        y: y,
        w: w,
...
function drawButton(button: UIButton) {
    if (button.kind === "card") {
...
function drawEmbers(seedOffset: number) {
...
function drawStatsBreakdown(panelY: number) {
