...
function pickSampleIndex(kind) {
    ...
    if (!bag || bag.length === 0) {
        ...
        const lastIndex = bags[kind + "_last"];
...
export const AudioFX = {
    ...
    tone: function (freq, duration, type, vol, slideTo, delay) {
        createContext();
        if (!ctx || !master) {
            return;
        }
    ...
    noise: function (duration, vol, frequency) {
        createContext();
        if (!ctx || !master) {
            return;
        }
